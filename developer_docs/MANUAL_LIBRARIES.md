...
scripts/
└── regenerate_sdks.py # Regenerates SDKs and updates exports and versions
...
- [ ] Run `python3 scripts/regenerate_sdks.py`
...
- Verify `scripts/regenerate_sdks.py` exists
- Check Python version: `python3 --version` (requires 3.6+)
- Run `python3 scripts/regenerate_sdks.py` so Fern generation and export updates stay together

## CI/CD Integration

The local generation script runs the update script automatically:

- `scripts/regenerate_sdks.py` - Updates exports during SDK regeneration

### Local Development Workflow

After running `python3 scripts/regenerate_sdks.py`:
...
**Note:** The `lib/` directories are protected by `.fernignore` but the main export files (`__init__.py`, `index.ts`) are regenerated by Fern. Always use `python3 scripts/regenerate_sdks.py` so export updates are applied afterward.
